...
import {
  loadPendingOperationalSignalEvent,
  markOperationalSignalFailure,
...
export async function processCatalogUpdate(
  ...
  const pendingOperational = await loadPendingOperationalSignalEvent(db, value.eventId);
  if (pendingOperational) {
    try {
      await processOperationalSignalEvent(db, pendingOperational);
    } catch (error) {
      await markOperationalSignalFailure(db, value.eventId, error);
      throw error;
    }
  }
